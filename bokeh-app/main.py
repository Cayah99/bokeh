...
# Creating the figure
p = figure(title = "Amount of kills over the years", height=700, width=900)
...
r1 = p.line([], [], color="lightgreen", line_width=3, legend_label="Attacks")
r2 = p.line([], [], color="purple", line_width=3, legend_label="Kills")
...
k.line('x', 'y', source = source1, color="lightgreen", line_width=3, legend_label="No success (0.0)")
k.line('x', 'y', source = source2, color="purple", line_width=3, legend_label="Success (1.0)")
...
select = Select(title='Choose Country:', value='Afghanistan', options=Countries_list, width=400)
...
grid1 = gridplot([[button], [p], [select, slider_range], [k, t]])

X_Features = df_Terrorist[['Year']]
polynomial_features= PolynomialFeatures(degree=3)
X_poly = polynomial_features.fit_transform(X_Features)
polynomial_features= PolynomialFeatures(degree=3)
X_pred = polynomial_features.fit_transform(np.array([2018,2019,2020,2021,2022,2023,2024,2025]).reshape(8,1)) 
Y= df_Terrorist['City']

Linear = LinearRegression()
Linear = Linear.fit(X_poly,Y)
y_pred = Linear.predict(X_poly)
y_pred2 = Linear.predict(X_pred)

#The visualization is created 
a= figure(title = "Amount of attacks in the future",  height = 600, width=800)
a.line(list(X_Features['Year']), y_pred, color = "purple", line_width=3,  legend_label="History")
a.line(np.array([2018,2019,2020,2021,2022,2023,2024,2025]), y_pred2, color = "lightgreen",line_width=3,  legend_label="Prediction" )
a.xaxis.axis_label = "Years"
a.yaxis.axis_label = "Amount of attacks"

grid2 = gridplot([[a]])

tab1 = Panel(child=grid, title='General')
tab2 = Panel(child=grid1,title="History of terrorism attacks in the world")
tab3 = Panel(child=grid2, title = 'Future of terrorism attacks in the world')
